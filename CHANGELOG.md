...
- **修复「配置访问权限授权后，网页目录浏览器仍点不开 /volN 层」**：fnOS 卷以 trimacl（btrfs 自定义 ACL）挂载，posix ACL（setfacl）在卷上不被内核执行——getfacl 可见、实际不生效，3.0.0.10~.12 的「祖先链 r-x」方案整体无效。现改为构建期补丁 #5：MP 目录浏览接口在无列举权层捕获 PermissionError，并按授权清单（fnos_grants.txt，由「配置访问权限」钩子实时维护）在祖先层合成「下一跳」虚拟目录——网页可从 / 逐级点开到授权目录本体（其内为原生真实读写）
- **过滤无权限子项**：目录浏览器不再显示应用用户无权访问的目录/文件（点开即空的浏览噪声）；授权链的虚拟跳层不受过滤影响
