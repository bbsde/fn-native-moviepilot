...
| fnOS 应用用户无权访问用户共享目录（3.0.0.10 定论） | fnOS 权限模型：卷根 `d---------`，一切靠 ACL；应用用户仅对自有 data-share 有精确条目 → MP 网页目录浏览器读不到 /volN 用户目录。**平台原生解**：fnOS 应用设置自带「配置访问权限」，按目录为应用用户授 ACL（真机实证：hermes 应用用户经此获得用户主目录 `--x` 条目）。另实证：**升级设备上配置面板新增的 wizard 字段不会回传给 config_callback**（旧字段正常、新字段 env 为空）——配置类新功能不能依赖加面板字段 | 面板 tips 引导用户去 fnOS 原生「配置访问权限」授权媒体库/下载目录（不再自造 setfacl 授权链）；README/发布帖 FAQ 同步说明 |
