...
| fnOS 应用用户无权访问用户共享目录（3.0.0.10 修复） | fnOS 权限模型：卷根 `d---------`，一切靠 ACL；应用用户仅对自有 data-share 有精确条目 → MP 网页目录浏览器读不到 /volN 用户目录，整理/转移更无从谈起。平台惯例即逐应用 ACL 授权（真机实证：hermes 应用用户在用户主目录有 `user:hermes:--x` 条目） | 应用设置面板（wizard/config）新增「目录授权」：用户填逗号分隔的目录路径 → config_callback（root）`setfacl`：目标递归 `rwx` + 默认 ACL（子目录继承），祖先链仅 `r-x`（浏览可达、范围外不可入）；记录落 `granted_dirs.txt`，uninstall_callback 逐条回收（含 default ACL 与祖先条目）；拒绝授权 / 与卷根 |
