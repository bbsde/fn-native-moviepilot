...
| 上游数据源国内可达性分层（2026-08 真机实测，拆梯子状态） | api.themoviedb.org：DNS 污染（解析到 Facebook/Dropbox 网段，TCP 不通）；**api.tmdb.org（TMDB 官方别名）：直连 0.7s 可用**；raw.githubusercontent.com：间歇性重置（4 测 2 断）→ 插件市场时好时坏；graphql.anilist.co：直连可用（首访慢是模块先试 trace.moe 中文代理被 403 再回退官方，另中文标题数据集 854KB 硬编码 raw 直连，实测 3.6~14.8s 且间歇性失败，为首访慢主因）；api.bgm.tv：DNS 污染 + **SNI 重置**双重封锁（裸 TCP 连 Cloudflare IP 通、带 SNI 的 ClientHello 立即 RST），纯 HTTP 80 与 bangumi.tv 家族同样被阻断；**社区镜像 api.bangumi.lol（Mirrox 开源反代）直连可用**，且返回 JSON 中图片域自动改写为 lain.bangumi.lol（实测 200），备选 bgmapi.anibt.net / bgmimg.anibt.net | install/upgrade 回调默认写入 `TMDB_API_DOMAIN='api.tmdb.org'`、`GITHUB_PROXY='https://ghproxy.net/'`、`BANGUMI_API_DOMAIN='api.bangumi.lol'`（均仅缺失时写入，用户已改的值绝不覆盖）；ghproxy.net 为当日实测最快镜像（ghfast.top 黑名单 / ghproxy.cn 返回插页 / mirror.ghproxy.com 超时），镜像失效时 设定→高级设置→网络 清除或更换即自动回退直连策略；Bangumi 依赖**构建期补丁 3**（BANGUMI_API_DOMAIN 可配置 + CONFIG_WATCH 热重建，上游硬编码 `_base_url`），第三方镜像可见查询内容（识别文件名/搜索词），介意者 app.env 改回 api.bgm.tv（需梯子）；AniList 依赖**构建期补丁 4**（中文数据集 URL 前缀拼接 GITHUB_PROXY，空值保持直连，CONFIG_WATCH 热重建）；另注：MP 内置 DoH 默认解析器（1.1.1.1/9.9.9.9 等）在国内本身不可达，勿指望它救污染域名 |
...
| 构建期补丁锚点策略（auto-follow 友好，3.0.0.8 固化） | 上游活跃开发期锚点易腐烂：注释文本/方法体/集合整行锚在上游加键或重构时断裂；补丁内复制上游 URL 会随上游改址静默 404（构建仍绿） | 三层策略：① 外层 guard 认「上游已采纳/已修复」→ 干净跳过（补丁1 缺失即跳；补丁2 认 `owner_class is type(self)`；补丁3 认 `BANGUMI_API_DOMAIN`；补丁4 认 `GITHUB_PROXY`）；② 内层锚点选低频变动锚——`def __init__` 行、`CONFIG_WATCH = {` 开括号子串（替换产生的重复 set 元素无害）、数据集 URL 由类属性前缀派生不复制；③ 结构性重构 → 构建 die → auto-follow 零发布待人工（fail-closed）。auto-follow 落地时 Release body 附补丁 applied/skipped 清单 |
| fnOS 应用用户无权访问用户共享目录（3.0.0.11 定案） | fnOS 权限模型：卷根 `d---------`，一切靠 ACL；应用用户仅对自有 data-share 有条目。原生「配置访问权限」对目标目录授读写（trim_acl，内核层生效、实测可写），**但祖先层仅 `--x` 穿透** → 目录浏览器打不开 /volN 层。实测钩子：配置访问权限变更 → `trim_app_center` 调 config_init（旧态）/config_callback（新态），**当前授权清单经 `TRIM_DATA_ACCESSIBLE_PATHS`（冒号分隔）传入**；package 模式下 config 脚本以应用用户运行（无特权）。另：升级设备上面板**新增** wizard 字段不回传 config_callback（旧字段正常）——配置类新功能不能依赖加面板字段 | 定案三层：① 读写授权 = fnOS 原生「配置访问权限」；② privilege 改 `run-as: root`，**生命周期提权、服务降权**（main 经 AS_APP_USER/runuser 以 moviepilot 跑前后端与网关桥，桥 pid 由内层 bash `echo $$`+exec 记录本体 pid）；③ 浏览层 = config_callback 读 `TRIM_DATA_ACCESSIBLE_PATHS` 为祖先链精确授 `r-x`（**授权即生效，无需重启**），install/upgrade 兜底同步（记录重放 browse_grants.txt + 当前清单），uninstall 按记录回收 |
